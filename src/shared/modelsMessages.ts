...
export const turnReady = async (num: number, conversation: conversationInterface, hourRange:Array<string>) => {
    ...
    const turns:any = (await Turn.find({"date":conversation.date, "place":conversation.place}).select({turns:{$elemMatch:{hour:new RegExp(`^${hourRange[0].split(":")[0]}`,"i"), reserved:false}}}))[0]?.turns
    ...
    1) Constancia de Inscripción BEEG vigente. (Firmado y Sellado por la institución escolar y policial). Obtenida del sitio oficial https://beg.misiones.gov.ar/#/home
