...
export const sendMessage =async (req:Request, res:Response) => {
    try {
        ...
        const text = whastappMessage.entry[0].changes[0].value.messages[0].text?.body || whastappMessage.entry[0].changes[0].value.messages[0].interactive?.type.list_reply?.tittle || "error"
